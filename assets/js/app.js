...
// ============================================
// CONFIG (加载 site.config.json，见 scripts/build.js)
// ============================================
app.config = {
    ...
    "links": [
      {
        ...
        "url": "https://github.com/Ameiro-sudo"
      },
      {
        "name": "Shizukuレモン",
        "icon": "fa7-brands/bilibili",
        "url": "https://space.bilibili.com/3493084421687360"
      },
      {
        "name": "博客",
  ...
  "ogDefaults": {
    ...
    "image": "https://blog.snowblock.top/assets/brand/og-image.png",
    "url": "https://blog.snowblock.top"
  }
}
...
app.utils = {
  ...
  stripFrontMatter: function(text) {
    // remove "# Title\n...yaml metadata...\n---\n" from markdown posts (兼容 CRLF 行尾)
  ...
  renderFiltered: function(filtered) {
    ...
    posts.forEach(function (p) {
      var tags = p.tags.map(function (t) {
        ...
        var safeT = app.modules.esc(t)
        return '<span class="' + cls + active + '" data-tag="' + safeT + '">' + safeT + '</span>'
      }).join('')
      var title = p.title
      var excerpt = app.state.allExcerpts[p.id] || ''
      if (app.modules && app.modules.esc) {
        title = app.modules.esc(title)
        excerpt = app.modules.esc(excerpt)
      }
...
app.gallery = {
  ...
  renderList: function() {
    ...
    grid.querySelectorAll('.album-card').forEach(function (el, idx) {
      el.style.animationDelay = Math.min(idx * (parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--motion-stagger')) || 0.08), 0.8) + 's'
      el.addEventListener('click', function () { location.hash = '#/gallery/' + el.dataset.album })
    })
  },
  ...
  renderBatch: function() {
    ...
    for (var i = this.loaded; i < end; i++) {
      (function (el, img, exif) {
        el.style.animationDelay = Math.min((i % this.batchSize) * (parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--motion-stagger')) || 0.06), 0.6) + 's'
        img.addEventListener('click', function () {
          app.lightbox.open(img.src, '', exif)
        })
      }).call(this, items[i], items[i].querySelector('img'), this.albumData.photos[i].exif)
    }
    this.loaded = end
...
app.router = {
  ...
  handleHash: function() {
    ...
    if (raw.indexOf('gallery/') === 0) {
      var albumId = null
      try { albumId = decodeURIComponent(raw.replace('gallery/', '')) } catch (e) { albumId = raw.replace('gallery/', '') }
      app.gallery.show()
